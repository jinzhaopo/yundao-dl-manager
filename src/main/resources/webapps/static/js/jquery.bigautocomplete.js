	...
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				...
				if($("#bigAutocompleteContent").css("display") != "none" && callback_ && $.isFunction(callback_)){
					//console.log($bigAutocompleteContent.find(".ct").data("jsonData"));
		...
		this.autocomplete = function(param){
			this.on("focus",function(e){
				...
				e.returnValue = false;
				//console.log(e);
				if($.trim($this.val()) == $.trim($this.config.defaultKeywords)){
            		$this.val("");
       			}
			});
			this.on("blur",function(){
				if($this.val() == ""){
					$this.val($this.config.defaultKeywords);
				}
			...
			$this.keyup(function(event) {
				...
				if(!isFunctionalKey && (!ctrl || (ctrl && k == 67) || (ctrl && k == 88)) ){
					...
					if(data != null && $.isArray(data) ){
						...
						makeContAndShow(data_, title);
					}else if(url != null && url != ""){//ajax请求数据
						$.post(url,{keyword:keyword_},function(result){
							makeContAndShow(result,title)
						},"json")
					}
			...
			//组装下拉框html内容并显示
			function makeContAndShow(data_, title){
