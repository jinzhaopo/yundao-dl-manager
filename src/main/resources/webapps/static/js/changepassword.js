LG.changepassword = function ()
{
    ...
    if (!window.changePasswordWin)
    {
        ...
        changePasswordPanle.ligerForm({
            fields: [
                ...
                { display: '确认密码', name: 'currentPassword2', type: 'password', validate: { maxlength: 50, required: true, equalTo: '#currentPassword', messages: { required: '请输入密码', equalTo: '两次密码输入不一致'}} }
            ]
        });
    ...
    function doChangePassword()
    {
        ...
        {
            LG.ajax({
                url: base + '/admin/user/updateProfile.jhtml',
                data: { "s_user.password": OldPassword, currentPassword: currentPassword },
                success: function ()
                {
